...
public class StrengthManager {
    ...
    /**
     * On death: lose exactly 1 strength. Never goes below 0.
     */
     ...
     * Returns false if amount is invalid or exceeds current strength.
     */
    ...
    private void setStrength(Player player, int value) {
        int clamped = Math.max(0, Math.min(MAX_STRENGTH, value));
     ...
     * Each strength point = +1 attack damage exactly.
    ...
    public void applyAttributeModifier(Player player, int points) {
        ...
        // ADD operation: flat +points to attack damage
        ...
        if (dataConfig.getConfigurationSection("strengths") != null) {
            for (String key : dataConfig.getConfigurationSection("strengths").getKeys(false)) {
                try {
                    ...
                    int val = Math.max(0, Math.min(MAX_STRENGTH, dataConfig.getInt("strengths." + key, 0)));
