...
public class StrengthListener implements Listener {
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        // ── Lose 1 strength on death ─────────────────────────────────────────
        int before = manager.getStrength(victim);
        manager.loseStrengthOnDeath(victim);
        int after = manager.getStrength(victim);

        if (before > after) {
            victim.sendMessage(buildPrefix() + "§cYou lost 1 Strength on death. "
                    + "§7(" + after + "/" + StrengthManager.MAX_STRENGTH + ")");
        }

        // ── Give shell to killer (must be a different player) ────────────────
